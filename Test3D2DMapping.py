...
def test_kitti():
    ...
    if is_disp:  # 如堕读取的是视差图
        if bgr.shape[:2] != disp.shape[:2]:
            cv2.resize(bgr, (disp.shape[1], disp.shape[0]),
                       interpolation=cv2.INTER_CUBIC)
            print('BGR image is resized to {:d}×{:d}.'.format(
                disp.shape[1], disp.shape[0]))
    else:
        if bgr.shape[:2] != depth.shape[:2]:
            cv2.resize(
                bgr, (depth.shape[1], depth.shape[0]), interpolation=cv2.INTER_CUBIC)
            print('BGR image is resized to {:d}×{:d}.'.format(
...
def test_pointcloud2depth():
    ...
    pc = o3d.io.read_point_cloud(ply_f_path)
    print(pc)
    pts3d = np.asarray(pc.points)
    print(pts3d)
    print('Total {:d} 3d points.'.format(pts3d.shape[0]))

    # 设置相机内参矩阵K
    K = np.array([[903.78201, 0.0, 646.90109],
                  [0.0, 903.60062, 310.24680],
                  [0.0, 0.0, 1.0]], dtype=np.float)
    print(K)

    # 计算2D坐标和深度
    res = np.dot(K, pts3d.T)  # 3D 投影到 2D
    pts2d_homo = res.T
    print(pts2d_homo)

    depths = pts2d_homo[:, 2]
    ply_f_name = os.path.split(ply_f_path)[-1]
    print('Depths of {:s}:\n'.format(ply_f_name), depths)

    U = pts2d_homo[:, 0] / pts2d_homo[:, 2]
    V = pts2d_homo[:, 1] / pts2d_homo[:, 2]
    print('U:\n', U)
    print('V:\n', V)
    ...
    uv_2_depth = defaultdict(list)
    for i, (u, v) in enumerate(zip(U, V)):
        print(u, v)
        uv_2_depth[(int(u), int(v))].append(depths[i])
    print(uv_2_depth)

    # Get depth map
    for v in range(480):
        for u in range(640):
            if (u, v) in uv_2_depth:
                depth_map[v, u] = np.min(uv_2_depth[u, v])

    depth_map_path = './{:s}_depth.png'.format(ply_f_name.split('.')[0])
    cv2.imwrite(depth_map_path, depth_map)
    print('{:s} saved.'.format(depth_map_path))
