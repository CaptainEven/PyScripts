...
def rename_files(file_root, width=2, prefix=None):
    """
    以编号重命名目录下所有文件
    """
    if not os.path.isdir(file_root):
    ...
    for i, file_path in enumerate(file_paths):
        ...
        if prefix is None:
            ...
            new_name = prefix + format.format(i) + '.' + suffix
...
def SubSampleVideo(video_f_path, tmp_frame_dir, interval=1):
    """
    每间隔interval帧采样视频序列
    """
    if not os.path.isfile(video_f_path):
        print('[Err]: ')
        return

    cap = cv2.VideoCapture(video_f_path)
    print('--processing video %s...' % video_f_path)
    FRAME_NUM = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))  # 获取视频所有帧数
    print('Total {:d} frames'.format(FRAME_NUM))

    if FRAME_NUM == 0:
        print('[Err]: zero frames.')
        return
    ...
    for i in range(0, FRAME_NUM):
        ...
        # 采样
        if i % interval == 0:
            print('Frame {:d} sampled.'.format(i))
            # frame_path = 
