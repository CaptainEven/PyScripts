...
def videos2frames(videos_list,
    ...
    for x in tqdm(videos_list):
        ...
        # seq_id = re.match('([a-zA-Z]+)_([a-zA-Z]+)_([\d]+)\.mp4',  # '([a-zA-Z]+)_([a-zA-Z]+)([\d]+)_?(\d*)\.mp4'
        #                   seq_name).group(3)
        # img_dir = img_root + '/mcmot_seq_imgs_' + str(seq_id)
...
def rotate_imgs(img_root, angle=-90.0):
    """
    """
    img_paths = [img_root + '/' +
    ...
    for i, img_path in enumerate(img_paths):
        ...
        # mat_rot = cv2.getRotationMatrix2D((h * 0.5, w * 0.5), angle, 1)
        # dst = cv2.warpAffine(src, mat_rot, (w, h*2))
        ...
        print('{:s} rotated in {:.3f}'.format(img_path, angle))
