...
def parse_car_multi_attrib_log(log_f_path):
    ...
    pattern = '.*Epoch[\s|\t]+([0-9]+).*train\s{1}acc\s{1}([0-9]+\.[0-9]+)%\s{1}\|test\s{1}acc\s{1}([0-9]+\.[0-9]+).*'
    ...
    with open(log_f_path, 'r', encoding='utf-8') as r_h:
        ...
        for line_i, line in enumerate(lines):
            ...
            if acc > max_acc:
                ...
                best_epoch = epoch
                the_line_i = line_i
                the_line = lines[the_line_i - 2]

        print(lines[the_line_i - 2], lines[the_line_i - 1])

    print('Max test acc {:.5f} @epoch {:d}'.format(max_test_acc, max_test_acc_epoch))
    print('Best train epoch: {:d}, acc {:.5f}'.format(best_epoch, max_acc))
    print(the_line)

    # 解析最终结果
    pattern = '.*(epoch\w+best\.pth).*'
    match = re.match(pattern, the_line)
    if match is not None:
        checkpoint = match.groups()[0]
        print(checkpoint)

if __name__ == '__main__':
    parse_car_multi_attrib_log(log_f_path='e:/2020_0814.log')
    print('Work done.')
