...
def batch_analysis(weights_list_file, image_list_file, thresh, iou_thresh, result_dir):
    ...
    for weights in weights_list:
        ...
        object_type = [meta.names[i].decode(
            'utf-8').strip() for i in range(meta.classes)]
        ...
        for j, image_path in enumerate(image_list):
            ...
            # 选择对应的dn
            det, time1 = dn.detect_ext(
                net, meta, bytes(image_path, 'utf-8'), thresh)
        ...
        for i, objtype in enumerate(object_type):
            ...
            # 此处为xywh(中心)，应该变为xmin,ymin,xmax,ymax
            ...
            for j, image_path in enumerate(image_list):
                ...
                for d in det:
                    xmin = float(copy.deepcopy(d[2])) - \
                        float(copy.deepcopy(d[4]))/2.0
                    ymin = float(copy.deepcopy(d[3])) - \
                        float(copy.deepcopy(d[5]))/2.0
                    xmax = float(copy.deepcopy(d[2])) + \
                        float(copy.deepcopy(d[4]))/2.0
                    ymax = float(copy.deepcopy(d[3])) + \
                        float(copy.deepcopy(d[5]))/2.0
                ...
                cmp_res = cdl.CmpData(
                    objtype, det, label, thresh, iou_thresh, img)
            ...
            if len(det_objtype) == 0:
                ...
                ap = voc_eval(det_objtype, annopath,
                              imagesetfile, objtype, iou_thresh)
            detall = []
            ...
            if total_label+total_detect-total_corr > 0:
                avg_acc = float(total_corr) / \
                    (total_label+total_detect-total_corr)
            avg_precision = 0
            if total_detect > 0:
                avg_precision = float(total_corr)/total_detect
            total_result = [total_label, total_detect, total_corr,
                            avg_recall, avg_iou, avg_acc, avg_precision]
            cdl.ExportAnaRes(objtype, cmp_result, total_result,
